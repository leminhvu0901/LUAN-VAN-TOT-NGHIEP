...
<div class="p-6 space-y-6">
    ...
    <div class="flex items-center gap-4 mb-4">
        ...
        <div>
            ...
            <p class="text-gray-500 text-sm mt-1">Tạo mã giảm giá, voucher mới cho khách hàng.</p>
        </div>
    </div>
                ...
                <div class="bg-white rounded-2xl organic-shadow border border-gray-100 p-4 sm:p-6">
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div id="max-discount-wrap">
                                ...
                                <input type="text" id="display-max-discount"
                                    value="{{ old('max_discount_amount') ? number_format(old('max_discount_amount'), 0, ',', '.') : '' }}"
                                    placeholder="Không giới hạn"
                                    class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all text-sm">
                                <input type="hidden" name="max_discount_amount" id="promo-max-discount" value="{{ old('max_discount_amount') }}">
                        ...
                        <div>
                            ...
                            <input type="text" name="description" id="promo-description" value="{{ old('description') }}"
                                maxlength="100" placeholder="VD: Giảm 20% cho đơn từ 100k"
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all text-sm">
                            <p class="text-xs text-gray-400 mt-1 flex justify-between"><span>Tối đa 100 ký tự.</span><span id="desc-count">{{ strlen(old('description', '')) }}/100</span></p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" id="display-min-order"
                                value="{{ old('min_order_amount') ? number_format(old('min_order_amount'), 0, ',', '.') : '' }}"
                                placeholder="Không giới hạn"
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all text-sm">
                            <input type="hidden" name="min_order_amount" id="promo-min-order" value="{{ old('min_order_amount') }}">
                        ...
                        <div>
                            ...
                            <select name="apply_for" class="custom-select-init w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all text-sm appearance-none bg-white" data-width-class="w-full">
                                ...
                                <option value="gold" {{ old('apply_for') == 'gold' ? 'selected' : '' }}>Vàng (Gold)</option>
                                <option value="diamond" {{ old('apply_for') == 'diamond' ? 'selected' : '' }}>Kim cương (Diamond)</option>
                            </select>
                            <p class="text-xs text-gray-400 mt-1">Chỉ những khách hàng đạt hạng này trở lên mới có thể sử dụng.</p>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">Kênh áp dụng</label>
                            <select name="applies_to" class="custom-select-init w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all text-sm appearance-none bg-white" data-width-class="w-full">
                                <option value="all" {{ old('applies_to', 'all') == 'all' ? 'selected' : '' }}>Tất cả (tại quầy + giao hàng)</option>
                                <option value="pickup" {{ old('applies_to') == 'pickup' ? 'selected' : '' }}>Chỉ đơn tại quầy</option>
                                <option value="delivery" {{ old('applies_to') == 'delivery' ? 'selected' : '' }}>Chỉ đơn giao hàng</option>
                            </select>
                            <p class="text-xs text-gray-400 mt-1">Mã "Tại quầy" được lễ tân tự động áp dụng, không cần khách nhập; mã "Giao hàng" khách tự nhập ở trang thanh toán.</p>
                        </div>
                        <div>
                ...
                <div class="bg-white rounded-2xl organic-shadow border border-gray-100 p-4 sm:p-6">
                    ...
                    <div class="mb-4 pb-4 border-b border-gray-100">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="is_recurring" id="is_recurring_cb" value="1" {{ old('is_recurring') ? 'checked' : '' }} class="w-4 h-4 text-emerald-600 rounded border-gray-300 focus:ring-emerald-500">
                    ...
                    <!-- Thời gian Lặp lại -->
                    <div id="recurring_time_wrapper" class="space-y-4 {{ old('is_recurring') ? '' : 'hidden' }}">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Lặp lại vào các ngày</label>
                            <div class="grid grid-cols-3 sm:grid-cols-4 gap-2">
                                @php $days = [1=>'T2', 2=>'T3', 3=>'T4', 4=>'T5', 5=>'T6', 6=>'T7', 7=>'CN']; @endphp
                                @foreach($days as $val => $label)
                                <label class="flex items-center gap-1 cursor-pointer">
                                    <input type="checkbox" name="recurring_days[]" value="{{ $val }}" 
                                        {{ is_array(old('recurring_days')) && in_array($val, old('recurring_days')) ? 'checked' : '' }}
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="time" name="recurring_start_time" value="{{ old('recurring_start_time') }}"
                            ...
                            <div>
                                ...
                                <input type="time" name="recurring_end_time" value="{{ old('recurring_end_time') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none text-sm">
                            </div>
                        </div>
                    </div>
...
<script src="{{ asset('js/backend/admin/promotions/create.js') }}"></script>
@endpush
