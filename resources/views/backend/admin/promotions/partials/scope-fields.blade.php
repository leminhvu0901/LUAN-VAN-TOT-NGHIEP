...
<div class="bg-white rounded-2xl organic-shadow border border-gray-100 p-4 sm:p-6">
    ...
    <div id="scope-bxgy-fields" class="{{ $selectedScope === 'buy_x_get_y' ? '' : 'hidden' }} space-y-5">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Sản phẩm cần mua <span
                        class="text-red-500">*</span></label>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Số lượng cần mua (X) <span
                        class="text-red-500">*</span></label>
                <input type="number" name="buy_quantity" min="1" value="{{ old('buy_quantity', $bxgy?->buy_quantity) }}"
                    placeholder="VD: 2"
            ...
            <div>
                ...
                <p class="text-xs text-gray-400 mt-1">Có thể chọn trùng với sản phẩm cần mua (vd: mua 2 tặng 1 cùng
                    loại).</p>
                @error('gift_product_id')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Số lượng tặng (Y) <span
                        class="text-red-500">*</span></label>
                <input type="number" name="gift_quantity" min="1"
                    value="{{ old('gift_quantity', $bxgy?->gift_quantity) }}" placeholder="VD: 1"
                    class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all text-sm">
                @error('gift_quantity')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                ...
                <input type="number" name="max_applications_per_order" min="1"
                    value="{{ old('max_applications_per_order', $bxgy?->max_applications_per_order) }}"
                    placeholder="Không giới hạn"
                    class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all text-sm">
                <p class="text-xs text-gray-400 mt-1">Vd: 1 = mua 4 vẫn chỉ tặng 1 lần. Để trống = mua càng nhiều tặng
                    càng nhiều.</p>
            </div>
            <div class="flex items-end">
                <label class="flex items-center gap-3 cursor-pointer">
                    <div class="relative">
                        <input type="checkbox" name="auto_add_gift" value="1" {{ old('auto_add_gift', $bxgy ? $bxgy->auto_add_gift : true) ? 'checked' : '' }} class="sr-only peer">
                        <div
