...
<div class="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 animate-fade-in-up">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 flex items-start sm:items-center gap-2.5">
                <span class="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-gradient-to-br from-emerald-100 to-emerald-200 text-emerald-600 shadow-inner border border-emerald-50 flex-shrink-0">
                    <span class="material-symbols-outlined text-2xl" style="font-variation-settings:'FILL' 1">payments</span>
                </span>
                <span class="bg-clip-text text-transparent bg-gradient-to-r from-gray-900 to-gray-600">Giá Trị Kho Hàng</span>
            </h1>
            <p class="text-sm text-gray-500 mt-2 sm:mt-1 sm:ml-[58px]">Theo dõi chi tiết giá trị của từng loại mặt hàng đang tồn trong kho.</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="stat-card group bg-gradient-to-br from-emerald-50 to-emerald-100/50 rounded-2xl p-5 border border-emerald-100/60 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 flex items-center gap-4 relative overflow-hidden">
            <span class="material-symbols-outlined text-8xl absolute -bottom-4 -right-4 text-emerald-500/5 group-hover:scale-110 group-hover:-rotate-6 transition-transform duration-500 select-none">category</span>
            ...
            <div class="z-10">
                ...
                <p class="text-3xl font-extrabold text-gray-900 tracking-tight" id="stat-count">{{ $materials->count() }}</p>
            </div>
        </div>

        {{-- Tổng giá trị kho --}}
        <div class="stat-card group bg-gradient-to-br from-emerald-50 to-emerald-100/50 rounded-2xl p-5 border border-emerald-100/60 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 flex items-center gap-4 sm:col-span-2 relative overflow-hidden">
            <span class="material-symbols-outlined text-8xl absolute -bottom-4 -right-4 text-emerald-500/5 group-hover:scale-110 group-hover:rotate-6 transition-transform duration-500 select-none">account_balance_wallet</span>
            <div class="w-12 h-12 rounded-2xl bg-white shadow-sm border border-emerald-100 flex items-center justify-center text-emerald-600 flex-shrink-0 group-hover:scale-110 transition-transform duration-300 z-10">
            ...
            <div class="min-w-0 z-10">
                ...
                <p class="text-3xl font-extrabold text-emerald-700 tracking-tight truncate" id="total-inventory-value">
    ...
    <div class="bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100/80 overflow-hidden flex flex-col">
        ...
        {{-- Toolbar --}}
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50 backdrop-blur-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
            ...
            <div class="flex items-center gap-2 flex-wrap">
                ...
                <div class="flex flex-col gap-1 w-full sm:w-auto">
                    ...
                    <div class="flex items-center gap-2 bg-white border border-gray-200 hover:border-emerald-300 rounded-xl px-3 py-2 shadow-sm group focus-within:border-emerald-500 focus-within:ring-4 focus-within:ring-emerald-500/10 transition-all duration-300 h-[42px] w-full sm:w-auto">
            ...
            <table id="table-inventory" data-total-id="total-inventory-value" data-highlight="bg-emerald-50" class="w-full text-left border-collapse whitespace-nowrap">
                ...
                <tbody class="divide-y divide-gray-50" id="table-body">
                    @forelse($materials as $material)
                        ...
                        @endphp
                        <tr class="hover:bg-emerald-50/40 transition-colors duration-200 group mat-table-row"
                            data-value="{{ $value }}"
                            ...
                            <td class="px-6 py-4">
                                <span class="bg-gray-100 text-gray-600 px-2 py-1 rounded-md text-xs font-bold font-mono">VT-{{ str_pad($material->id, 2, '0', STR_PAD_LEFT) }}</span>
                            </td>
                            
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-emerald-50 to-emerald-100 border border-emerald-100 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-300">
                                        <span class="material-symbols-outlined text-emerald-600 text-[18px]" style="font-variation-settings:'FILL' 1">inventory_2</span>
                                    </div>
                                    <span class="font-bold text-sm text-gray-900">{{ $material->name }}</span>
                                </div>
                            </td>
