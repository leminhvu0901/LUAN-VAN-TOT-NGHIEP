...
class OrderService
{
    ...
    public function create(User $user, array $payload, string $paymentMethod): Order
    {
        ...
        if (!$cart) throw ValidationException::withMessages(['cart' => 'Giỏ hàng của bạn đang trống.']);
        ...
        if ($quote['distance_km'] > ShippingQuoteService::MAX_DELIVERY_KM) {
            throw ValidationException::withMessages(['address_id' => 'Địa chỉ vượt quá phạm vi giao hàng 10 km.']);
        }
        ...
        return DB::transaction(function () use ($user, $payload, $paymentMethod, $address, $cart, $previewSubtotal, $quote) {
            ...
            $discount = min($subtotal, $couponDiscount + $membershipDiscount);
    ...
    {
        if (!filled($code)) return [null, 0];

        $promotion = Promotion::query()->where('code', strtoupper(trim($code)))->lockForUpdate()->first();
        if (!$promotion) throw ValidationException::withMessages(['coupon_code' => 'Mã giảm giá không tồn tại.']);
        $validity = $promotion->checkValidity($user, $subtotal);
        if (!$validity['valid']) throw ValidationException::withMessages(['coupon_code' => $validity['message']]);

        $discount = $promotion->type === 'percent'
            ? round($subtotal * ((float) $promotion->value / 100))
            : (float) $promotion->value;
        if ($promotion->max_discount_amount) $discount = min($discount, (float) $promotion->max_discount_amount);
