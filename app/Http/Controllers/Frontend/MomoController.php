...
class MomoController
{
     ...
     * với VNPay) rồi điều hướng sang cổng MoMo. Theo mẫu: php/PayMoMo/init_payment.php từ
     * github.com/momo-wallet/payment
     */
    public function createPayment(Request $request)
    {
        $result = $this->createPendingOrderForOnlinePayment($request, 'momo');
        if (!($result instanceof \App\Models\Order)) {
            return $result;
        }
        $order = $result;

        // ── Gọi API MoMo (theo mẫu PayMoMo/init_payment.php) ─────────────
        ...
        $payUrl = $this->requestPayUrl($order->order_code, (float) $order->final_amount, $orderInfo, 'payWithMethod');
        ...
        $this->rollbackPendingOnlinePaymentOrder($order);
    ...
    {
        if (!$this->configValid) {
            return $this->refundError($request, 'Chưa cấu hình MoMo cho môi trường chính thức. Vui lòng liên hệ quản trị viên.');
        }
        ...
        if ($order->payment_method !== 'momo' || $order->payment_status !== 'paid') {
            return $this->refundError($request, 'Đơn hàng này không cần hoàn tiền MoMo.');
        }
        ...
        if (!$result['success']) {
            ...
            return $this->refundError($request, 'Hoàn tiền MoMo thất bại: ' . $result['message']);
        }
