    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax() || $request->has('ajax')) {
            return response()->json([
                'table_html' => view('backend.orders.partials.table', compact('orders', 'paginator', 'status'))->with('currentStatus', $status)->render(),
                'stats_html' => view('backend.orders.partials.stats', compact('stats'))->render(),
            ]);
        }
        return view('backend.orders.index', compact('stats', 'orders', 'paginator'))->with('currentStatus', $status);
    }
