 ...
 * đánh giá sản phẩm — dùng chung 1 endpoint (ProductController::reviews()) cho cả trang chi tiết sản
 * phẩm (view=full) và trang "Xem đánh giá" (view=compact).
 */
class ProductReviewsFilterTest extends TestCase
{
    ...
    public function test_reviews_endpoint_filters_by_rating(): void
    {
        ...
        $response = $this->get("/products/{$product->id}/reviews?rating=5&view=full");
    ...
    public function test_reviews_endpoint_filters_by_has_image(): void
    {
        ...
        $response = $this->get("/products/{$product->id}/reviews?has_image=1&view=compact");
    ...
    public function test_reviews_endpoint_without_filter_returns_all_visible_reviews(): void
    {
        ...
        $response = $this->get("/products/{$product->id}/reviews?view=full");
    ...
    public function test_reviews_endpoint_paginates_and_load_more_reflects_remaining_pages(): void
    {
        ...
        $page1 = $this->get("/products/{$product->id}/reviews?view=full");
        $page1->assertOk();
        $page1->assertSee('data-next-page="2"', false);
        $this->assertEquals(5, substr_count($page1->getContent(), 'pd-review-item'));

        $page2 = $this->get("/products/{$product->id}/reviews?view=full&page=2");
        $page2->assertOk();
    ...
    public function test_you_tag_shown_only_for_own_review_and_only_when_logged_in(): void
    {
        ...
        $guestResponse = $this->get("/products/{$product->id}/reviews?view=full");
        ...
        $response = $this->actingAs($me)->get("/products/{$product->id}/reviews?view=full");
        $response->assertOk();
