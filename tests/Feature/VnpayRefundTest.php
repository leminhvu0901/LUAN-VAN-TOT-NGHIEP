...
class VnpayRefundTest extends TestCase
{
    ...
    /**
     * Regression: đổi admin.orders.refund/staff.reception.orders.refund sang dispatcher
     * (OnlinePaymentGatewayController) KHÔNG được làm vỡ refund MoMo hiện có — vẫn phải route đúng
     * sang MomoController::refundOrder() dựa theo payment_method của đơn.
     */
    public function test_refund_dispatcher_still_routes_momo_orders_to_momo_controller(): void
    {
        Http::fake([
            '*/v2/gateway/api/refund' => Http::response(['resultCode' => 0, 'transId' => 'MOMO-REFUND-TX'], 200),
        ]);
        ...
        $order = $this->makeOrder([
            ...
            'payment_status' => 'paid',
            'paid_at' => now(),
            'payment_transaction_id' => 'ORIGINAL-MOMO-TX',
        ]);
        ...
        $order = $order->fresh();
        $this->assertEquals('refunded', $order->payment_status);
        $this->assertEquals('MOMO-REFUND-TX', $order->refund_transaction_id);
    }
}
