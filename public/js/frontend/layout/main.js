...
const updateCartUI = (data) => {
    ...
    // 3. Không cập nhật cart-drawer-total ở đây nữa — sẽ do syncCartSelectionUI() tính theo sản phẩm đã chọn
    ...
    if (list) {
        if (data.count === 0) {
            ...
            data.items.forEach(item => {
                ...
                // Giá item = đơn giá × số lượng (dùng cho checkbox data-price)
                let itemTotal = item.unit_price * item.quantity;

                // Ghép HTML cho 1 sản phẩm trong giỏ — CÓ THÊM CHECKBOX
                html += `
            ...
            // Gắn sự kiện master checkbox (Chọn tất cả)
            ...
            // Cập nhật UI ban đầu sau khi render xong
            syncCartSelectionUI();
        }
    }
};
...
/**
 * Đồng bộ UI chọn lựa: tính lại tổng tiền đã chọn, cập nhật nút checkout, master checkbox.
 * Gọi mỗi khi một checkbox con thay đổi trạng thái.
 */
window.syncCartSelectionUI = function () {
    ...
    // Cập nhật master checkbox: checked nếu tất cả được chọn; indeterminate nếu một phần
    if (masterChk) {
...
/**
 * Lấy danh sách cart_item_id đang được tick chọn.
 * @returns {number[]}
 */
 ...
 * Gọi API POST /cart/set-selected để lưu danh sách đã chọn vào session,
 * sau đó điều hướng sang trang /checkout.
 */
