...
function toggleLocPanel() {
    ...
    if (isHidden) {
        ...
        if (list && !list.dataset.filled) {
            list.innerHTML = '<div class="p-4 text-xs text-on-surface-variant text-center">Vui lòng chọn vị trí trên bản đồ hoặc bấm "Định vị GPS" bên dưới để tự động điền khu vực.</div>';
            list.dataset.filled = '1';
        }
    }
}

// Chưa có bộ dữ liệu Tỉnh/Thành - Phường/Xã để duyệt/lọc thủ công trong locList; khu vực hiện được
// điền tự động qua bản đồ/GPS (xem reverseGeocode). Giữ hàm này để ô tìm kiếm không lỗi khi gõ.
function filterLocItems(query) {
}
    ...
    fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&accept-language=vi&addressdetails=1`)
        ...
        .then(data => {
            if (data && data.address) {
                ...
                // Tự động điền "Địa chỉ cụ thể" (số nhà + tên đường) theo vị trí vừa chọn trên bản đồ/GPS
                // — người dùng vẫn sửa lại được bình thường nếu geocode không chính xác.
                const street = addr.road || addr.pedestrian || addr.footway || addr.residential || '';
                const specificParts = [addr.house_number, street].filter(Boolean);
                const specificEl = document.getElementById('addr_specific');
                if (specificEl && specificParts.length > 0) {
...
function updateLocPickerText() {
    ...
    if (inputEl) {
        ...
        inputEl.value = parts.length > 0 ? parts.join(', ') : '';
    }
}
...
function openAddressModal(isEdit = false, data = null) {
    ...
    if (isEdit && data) {
        ...
        locState.province_name = data.province || '';
        locState.district_name = data.district || '';
        locState.ward_name = data.ward || '';
        updateLocPickerText();
    ...
    // Modal đang từ "hidden" (display:none) hiện ra -> phải khởi tạo/định vị lại bản đồ SAU khi
    // container đã hiển thị, nếu không Leaflet sẽ tính kích thước sai (bản đồ xám/vỡ layout).
    setTimeout(function () {
        initMapIfNeeded();
        if (map) {
            map.invalidateSize();
...
function saveAddress() {
    ...
    if (!locState.province_name || !locState.district_name || !locState.ward_name) {
        alert("Vui lòng chọn vị trí trên bản đồ hoặc bấm \"Định vị GPS\" để xác định khu vực (Tỉnh/Thành, Phường/Xã).");
        return;
    }
